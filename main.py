...
@click.option('--outputname', prompt='Desired output filename:', default='myawesomefile', help='filename no extension')
@click.option('--outputtype', prompt='Desired output file type:', default='json', help='Output type: json, xml')
@click.option('--name', prompt='Your name',
              help='The person to greet.')
def initConfig(count, name, outputname, outputtype):
    """Simple program that greets NAME for a total of COUNT times."""
    for x in range(count):
        click.echo(f"Hello {name}!")
        click.echo(f"Desired output file is {outputname}.{outputtype}!")
    ...
    if not isDev:
        while True:
            ...
            if full_response['pagination']['count'] == 0:
                ...
                print(utilities.esc('45;1;4') + 'processing ' + str(
                    full_response['pagination']['count']) + ' items.' + utilities.esc(0))
